<footer>
	<div id="footer_central">
    	<div class="redes_sociais">
        	<ul>
            	<li><a id="facebook" href="#"></a></li>
                <li><a id="twitter" href="#"></a></li>
            </ul>
            <ul>
            	<li><a id="instagram" href="#"></a></li>
                <li><a id="youtube" href="#"></a></li>
                <li><a id="linkedin" href="#"></a></li>
            </ul>
            <ul class="up_redes_sociais">
            	<li><a id="gmais" href="#"></a></li>
                <li><a id="orkut" href="#"></a></li>
            </ul>
        </div>
    	<div class="footer_menu">
            <ul>
                <li class="title_footer"><a href="#">Companhia</a></li>
                <li><a href="#">Blog</a></li>
                <li><a href="#">Team</a></li>
                <li><a href="#">Investors</a></li>
                <li><a href="#">News Room</a></li>
                <li><a href="#">Events</a></li>
                <li><a href="#">Contact Us</a></li>
            </ul>
            <ul>
                <li class="title_footer"><a href="#"> Resources</a></li>
                <li><a href="#">FOSS Program</a></li>
                <li><a href="#">Case Studies</a></li>
                <li><a href="#">Whitepapers</a></li>
                <li><a href="#">Recordings</a></li>
                <li><a href="#">Collateral</a></li>
            </ul>
            <ul>
                <li class="title_footer"><a href="#">Products</a></li>
                <li><a href="#">Jenkins</a></li>
                <li><a href="#">On-Premise</a></li>
                <li><a href="#">Public Cloud</a></li>
                <li><a href="#">Pricing</a></li>
            </ul>
            <div id="logo_footer" class="logo">
            	<a href="#" title="KcalControl">
                	<h1>KcalControl</h1>
                </a>
            </div>
        </div>
    </div>
</footer>